...
router.post('/', async (req, res) => {
  ...
  let cartItem = await CartItem.findOne({ where: { productId, userId: req.user.id } });
  if (cartItem) {
    if (cartItem.quantity + quantity > 10) {
      return res.status(400).json({ error: 'Quantity must not exceed 10' });
    }
...
router.put('/:productId', async (req, res) => {
  ...
  const cartItem = await CartItem.findOne({ where: { productId, userId: req.user.id } });
  if (!cartItem) {
